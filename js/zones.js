/**
 * zones.js v2.0
 * Зоны тела с определением попаданий и подсчётом ударов
 */
...
const Zones = (() => {
  ...
  const CFG = {
    ...
    headPad:     0.38,
    bodyPad:     0.12,
    hitFlash:    500, // мс свечения при ударе
  };
  ...
  function _headBox(kps) {
    const pts = [KP.NOSE, KP.L_EYE, KP.R_EYE, KP.L_EAR, KP.R_EAR]
      .map(i => kps[i]).filter(p => p && p.score >= CFG.minScore);
    ...
    return { x: minX-pad, y: minY-pad*1.8, w: w+pad*2, h: h+pad*3 };
  }
  ...
  function _bodyBox(kps) {
    ...
    return { x: minX-pad, y: minY-pad*0.5, w: (maxX-minX)+pad*2, h: (maxY-minY)+pad };
  }
  ...
  function draw(ctx, keypoints) {
    ...
    // Проверяем попадания запястий в зоны
