const UI = (() => {
  ...
  function setCameraStatus(s) {
    _setStatus(els.statusCamera, s, camLabels[s]);
  }
  function setModelStatus(s) {
    _setStatus(els.statusModel, s, modLabels[s]);
  }
  function setPersonStatus(s) {
    _setStatus(els.statusPerson, s, persLabels[s]);
  }
  function setFaceStatus(s) {
    _setStatus(els.statusFace, s, faceLabels[s]);
  }

  function setFPS(v) {
    if (els.fpsValue) els.fpsValue.textContent = v;
  }
  ...
  function setStartEnabled(v) {
    if (els.btnStart) els.btnStart.disabled = !v;
  }
  function setStopEnabled(v) {
    if (els.btnStop) els.btnStop.disabled = !v;
  }
  ...
  function setExportEnabled(v) {
    if (els.btnExport) els.btnExport.disabled = !v;
  }
  ...
  function updateScore(state) {
    ...
    if (els.scoreBig) {
      ...
      els.scoreBig.style.textShadow =
        state.combo > 2 ? "0 0 40px rgba(255,107,53,0.9)" : "";
    }
  ...
  function onToggleChange(callback) {
    [
      ...
      document
        .getElementById(id)
        ?.addEventListener("change", () => callback(getToggles()));
    });
  }
